...
DB_NAME = 'kreedos'
TABLE_NAME = 'ban_macro_mapping'
ARCHIVE_TABLE_NAME = 'ban_macro_mapping_archive'  # Optional

S3_BUCKET = 'kredos-uscellular-staging'
S3_FOLDER = 'kredos-uscellular-staging/ban-macro-mapping-archives/'
...
def delete_old_records(connection, record_ids):
    ...
    with connection.cursor() as cursor:
        ...
        query = f"DELETE FROM {TABLE_NAME} WHERE ban_macro_mapping_id IN ({placeholders})"
...
def main():
    ...
    export_filename = f"ban_macro_mapping_{today.strftime('%Y%m%d')}.csv"
    ...
    try:
        ...
        record_ids = [record['ban_macro_mapping_id'] for record in records]
